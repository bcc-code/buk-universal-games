...
export default class NotificationService {
  ...
  constructor(expiryTimeOffset = 300_000) {
    setInterval(() => {
      ...
      if (
        latestNotification &&
        !this.shownNotifications.find(
          (n) => n.time.getTime() === latestNotification.time.getTime(),
        ) &&
        new Date().getTime() <
          latestNotification.time.getTime() + expiryTimeOffset
      ) {
  ...
  schedule(time, title, options) {
    ...
    if (
      !this.shownNotifications.find((n) => n.time.getTime() === time.getTime())
    ) {
      ...
      this.scheduledNotifications = this.scheduledNotifications
        ...
        .filter(
          (n, ix, all) =>
            all.findIndex((n2) => n2.time.getTime() === n.time.getTime()) ===
            ix,
        );
    }
  }
